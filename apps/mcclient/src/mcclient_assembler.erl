...
init([Meta]) ->
    ...
    case file:open(Filename, [write, binary, {delayed_write, ?BUFFER_SIZE, ?DELAY}]) of
        {error, Err} ->
...
handle_info(tick, State) when State#state.key == '$end_of_table' ->
    ...
    if State#state.written < Meta#metadata.size -> handle_info(tick, State#state{key=0});
       true ->
            case calc_md5(State#state.filename) == Meta#metadata.md5 of
                false ->
                    ?LOG("Invalid file checksum, retrying!~n"),
                    init_info_table(Meta),
                    handle_info(tick, State#state{key=0, written=0});
                true ->
